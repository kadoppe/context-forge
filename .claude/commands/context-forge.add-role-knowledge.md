...
**重要**: version フィールドには context-forge のバージョンを使用してください。
バージョンは `pyproject.toml` の `version` フィールドから取得します。

1. プロジェクトルートの `pyproject.toml` を読み込む
2. `version = "X.Y.Z"` の値を抽出
3. plugin.json の version に設定

```json
{
  "name": "context-forge.role-{role-name}",
  "description": "{role-description}",
  "version": "{pyproject.toml の version 値}",
  "author": {
