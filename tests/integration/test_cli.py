...
@pytest.fixture
def in_temp_dir(tmp_path: Path) -> Generator[Path, None, None]:
...
def test_init_command_creates_directories_and_installs(in_temp_dir: Path) -> None:
    """Test that init creates directories and installs all commands by default."""
    result = runner.invoke(app, ["init"])
    assert result.exit_code == 0

    # Check that directories were created
    claude_dir = in_temp_dir / ".claude"
    commands_dir = in_temp_dir / ".claude" / "commands"
...
def test_init_command_skip_install(in_temp_dir: Path) -> None:
...
def test_init_command_existing_directory(in_temp_dir: Path) -> None:
...
def test_init_command_overwrite_prompt(in_temp_dir: Path) -> None:
...
def test_init_command_force_flag(in_temp_dir: Path) -> None:
...
def test_init_creates_context_forge_md(in_temp_dir: Path) -> None:
    """Test that init creates .claude/context-forge.md file."""
    result = runner.invoke(app, ["init", "--skip-install"])
    assert result.exit_code == 0

    # Check that context-forge.md was created
    context_forge_md = in_temp_dir / CONTEXT_FORGE_MD_PATH
...
def test_init_creates_claude_md_with_reference_when_not_exists(
    in_temp_dir: Path,
) -> None:
    ...
    claude_md = in_temp_dir / "CLAUDE.md"
...
def test_init_adds_reference_to_existing_claude_md(in_temp_dir: Path) -> None:
...
def test_init_skips_adding_duplicate_reference(in_temp_dir: Path) -> None:
...
"""
    ...
    (in_temp_dir / ".claude").mkdir(parents=True, exist_ok=True)
    (in_temp_dir / CONTEXT_FORGE_MD_PATH).write_text("# test", encoding="utf-8")
...
def test_init_output_shows_created_files(in_temp_dir: Path) -> None:
...
def test_claude_md_reference_is_under_10_lines(in_temp_dir: Path) -> None:
